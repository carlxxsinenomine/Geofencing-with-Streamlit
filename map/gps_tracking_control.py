...
class GPSTrackingControl(MacroElement):
    _template = Template("""
        {% macro script(this, kwargs) %}
            ...
            // To check if GPS is currently running
            ...
            // Track which fences have been breached to prevent duplicate alerts
            var breachedFences_{{ this.get_name() }} = new Set();
            // Track the last time an alert was shown (per fence)
            var lastAlertTime_{{ this.get_name() }} = {};
            // Cooldown period in milliseconds (e.g., 60 seconds)
            var alertCooldown_{{ this.get_name() }} = 60000;
            ...
            function checkGeofences_{{ this.get_name() }}(userLng, userLat) {
                ...
                for(var i = 0; i < drawnShapes_{{ this.get_name() }}.length; i++) {
                    ...
                    var isInside = false;
                    var fenceName = properties.name || 'Unnamed fence';
                    ...
                    if (isInside) {
                        currentlyInside.add(fenceName);

                        // Check if this is a new breach (not previously breached or cooldown expired)
                        var currentTime = Date.now();
                        var lastAlert = lastAlertTime_{{ this.get_name() }}[fenceName] || 0;
                        var timeSinceLastAlert = currentTime - lastAlert;

                        if (!breachedFences_{{ this.get_name() }}.has(fenceName) || 
                            timeSinceLastAlert > alertCooldown_{{ this.get_name() }}) {
                            breachedFences.push(fenceName);
                            lastAlertTime_{{ this.get_name() }}[fenceName] = currentTime;
                        }
                    }
                }

                // Find fences that were previously breached but user has now exited
                var exitedFences = [];
                breachedFences_{{ this.get_name() }}.forEach(function(fenceName) {
                    if (!currentlyInside.has(fenceName)) {
                        exitedFences.push(fenceName);
                    }
                });

                // Update the breached fences set
                breachedFences_{{ this.get_name() }} = currentlyInside;

                return { breachedFences: breachedFences, exitedFences: exitedFences };
            }

            function sendAlertToServer_{{ this.get_name() }}(fenceName) {
            ...
            function sendDataToServer_{{ this.get_name() }}() {
                if (pathCoordinates_{{ this.get_name() }}.length === 0) {
            ...
            function startTracking_{{ this.get_name() }}() {
                ...
                watchId_{{ this.get_name() }} = navigator.geolocation.watchPosition(
                    function(position) {
                        ...
                        // Check geofences and get breached/exited fences
                        var result = checkGeofences_{{ this.get_name() }}(lng, lat);

                        // Handle newly breached fences
                        result.breachedFences.forEach(function(fenceName) {
                            console.log('User entered fence:', fenceName);
                            alert("User entered " + fenceName);
                            sendAlertToServer_{{ this.get_name() }}(fenceName);
                        });

                        // Optionally handle exited fences
                        result.exitedFences.forEach(function(fenceName) {
                            console.log('User exited fence:', fenceName);
                        });
            ...
            function stopTracking_{{ this.get_name() }}() {
                ...
                if (locationMarker_{{ this.get_name() }} !== null) {
                    {{ this._parent.get_name() }}.removeLayer(locationMarker_{{ this.get_name() }});
                    locationMarker_{{ this.get_name() }} = null;
                }

                if (accuracyCircle_{{ this.get_name() }} !== null) {
                    {{ this._parent.get_name() }}.removeLayer(accuracyCircle_{{ this.get_name() }});
                    accuracyCircle_{{ this.get_name() }} = null;
                }
                ...
                // Reset breach tracking when stopping
                breachedFences_{{ this.get_name() }}.clear();
                lastAlertTime_{{ this.get_name() }} = {};
    ...
    def __init__(self, state, user_id=None):
        ...
        self._state_len = len(state) if state else 0
